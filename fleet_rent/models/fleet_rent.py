...
class FleetRent(models.Model):
    ...
    additional_rent_amt = fields.Monetary(
        "Additional Rental Amount",
        currency_field="currency_id",
        help="Additional Rental Amount  \
                                    For Exceeding Distance And Number Of Days.",
        copy=False,
    )
    days_exceeded = fields.Integer('Days Exceeded')
    new_odometer = fields.Float('New Odometer')
    allowed_distance = fields.Float("Allowed Distance")
    ...
    def action_receipt_car(self):
        for rec in self:
            rec.date_close = fields.Date.today()
            delta = rec.date_close.date() - rec.date_start.date()
            rec.days_exceeded = int(delta.days)
            print('delta---------->', int(delta.days))
            if int(delta.days) >= 1:
                td = rec.new_odometer - (rec.odometer + (int(delta.days) * rec.allowed_distance))
                tx = td * rec.additional_rent_amt
                rec.total_rent = tx
                # print('tx---------->', tx)
            else:
                td = rec.new_odometer - (rec.odometer + rec.allowed_distance)
                tx = td * rec.additional_rent_amt
                rec.total_rent = tx
                # print('tx---------->', tx)

            # al_per_days = delta * rec.allowed_distance
            # if delta > 1:
            #     rem_km = rec.new_odometer - al_per_days
            #     tx = rem_km * rec.additional_rent_amt
            # if delta > 1:
            #     if rec.new_odometer > rec.odometer + rec.allowed_distance:
            #         nk = rec.new_odometer - (rec.odometer + rec.allowed_distance)
            #         tax_per_km = nk * rec.additional_rent_amt
            #
            #         tax_per_extra_days = delta * 100
            #         tot_tax = tax_per_km + tax_per_extra_days
            # else:
            #     if rec.new_odometer > rec.odometer + rec.allowed_distance:
            #         nk = rec.new_odometer - (rec.odometer + rec.allowed_distance)
            #         taxed_per_km = nk * rec.additional_rent_amt
